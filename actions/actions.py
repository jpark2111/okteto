...
dataset = load_dataset(
    "text",
    data_files=[
        "https://huggingface.co/datasets/WealthBuild/Investing_Knowledgebase/resolve/main/529_Plans.txt",
        "https://huggingface.co/datasets/WealthBuild/Investing_Knowledgebase/resolve/main/ROTH_IRA.txt",
        "https://huggingface.co/datasets/WealthBuild/Investing_Knowledgebase/resolve/main/401k_Glossary_of_Terms.txt",
        "https://huggingface.co/datasets/WealthBuild/Investing_Knowledgebase/resolve/main/What_Is_a_401k_Plan.txt",
    ],
    use_auth_token=hf_token,
)["train"]["text"]
string_dataset = " ".join(dataset).strip("\n")
# loader = TextLoader(file_from_local)
# documents = loader.load()
# text_splitter = CharacterTextSplitter(
#     chunk_size=500, chunk_overlap=50, separator="\n\n", length_function=len
# )
# text_splitter = SpacyTextSplitter(pipeline="en_core_web_md")
...
# model = HuggingFaceEmbeddings(mod
#     "sentence-transformers/all-MiniLM-L6-v2", "embed_documents"
# )
# data_embeddings = model.encode(dataset)
...
retreiver = db.as_retriever(search_type="similarity", search_kwargs={"k": 8})
...
hub_prompt_template = """You’re an expert financial coach.  You have been helping people with financial planning for 20 years. Your task is now to give the best advice if someone ask question. If you don't know the answer that's fine.  The question given will be related to financial term. Please generate answers with more details in less than 100 words if possible. You must ALWAYS ask questions after you give the advice. Limit questions to 2 maximum to better understand how to best answer the user questions. Is that understood?
No Hallucinations, please quote source when possible.
Question: {question}

Answer: """

hub_follow_prompt_template = """You’re an expert financial coach.  You have been helping people with financial planning for 20 years. Your task is now to give the best advice if someone ask question. If you don't know the answer that's fine.  The message and response given below will be related to financial term. The response given below is a response from the message given below. Please generate answers with more details within same context with message and response given in less than 100 words if possible. You must ALWAYS ask questions after you give the advice. Limit questions to the userto 2 maximum to better understand how to best answer the user questions. Is that understood?
No Hallucinations, please quote source when possible.
...
class ActionGeneralLangChain(Action):
    ...
    def run(
        ...
        if (
            ...
            llm_chain = LLMChain(
                llm=llm, prompt=PromptTemplate.from_template(hub_prompt_template)
            )
            result = llm_chain.run(question=latest_bot_message)
        elif (
            previous_action_name == "action_general_langchain"
            and latest_intent != "affirm"
        ):
            llm_chain = LLMChain(
                llm=llm, prompt=PromptTemplate.from_template(hub_follow_prompt_template)
            )
            result = llm_chain.run(message=latest_bot_message, response=user_message)
        elif latest_intent == "knowledge_question" or "nlu_fallback":
            query = user_message
            llm_chain = LLMChain(
...
# class ActionFollowUpLangChain(Action):
#     def name(self) -> Text:
#         return "action_follow_langchain"
...
#         latest_bot_message = tracker.get_last_event_for(event_type="bot")["text"]
#         print(query)
#         print(latest_bot_message)
#         prompt_template = """Generate the question based on the context below. I'll give you previous
#             question and answer, then you need to ask user if they want to or need to know more about or make a move based on the question and answer I give you. The question you will generate should be in same context of question I give you.
...
#             llm=llm, prompt=PromptTemplate.from_template(prompt_template)
#         )
#         result = llm_chain.run(question=query, answer=latest_bot_message)
#         dispatcher.utter_message(text=result)
#         return []


class ActionHubLangChain(Action):
    def name(self) -> Text:
        return "action_hub_langchain"
    ...
    def run(
        ...
        # prompt_template = """You are a helpful assistant and advisor that specialized in finance to give answers. If you don't know the answer that's fine. First, summarize the question, understand the main topic of the question, then generate the answer on the question.

        # Question: {question}
        # Answer:"""
        prompt_template = """You are a helpful assistant and advisor that specialized in finance to give answers. If you don't know the answer that's fine. The question given below will be related to financial term, please generate answers in less than 100 words if possible.

        Question: {question}
        Answer: """

        # PROMPT = PromptTemplate(template=prompt_template, input_variables=["question"])
        # PROMPT.format(question=latest_bot_message)
        ...
        # )
        # result = qa.run(latest_bot_message)
        llm_chain = LLMChain(
            llm=llm, prompt=PromptTemplate.from_template(prompt_template)
        )
        result = llm_chain.run(question=latest_bot_message)
        ...
        dispatcher.utter_message(text=result)
        return [FollowupAction("action_hub_follow_langchain")]


class ActionHubFollowLangChain(Action):
    def name(self) -> Text:
        return "action_hub_follow_langchain"
    ...
    def run(
        ...
        prompt_template = """You are a helpful assistant and advisor that specialized in finance to generate a question. If you don't know what you need to generate that's fine. Generate the question based on the context below. Here is answer, you need to ask user if they want to or need to know more about or make a move based on the answer I give you. The question you will generate should be in same context of answer I give you.
        ...
        result = llm_chain.run(answer=latest_bot_message)
        dispatcher.utter_message(text=result)
        return []


class ActionFollowLangChain(Action):
    def name(self) -> Text:
        return "action_follow_langchain"
    ...
    def run(
        ...
        prompt_template = """ You’re an expert financial coach.  You have been helping people with financial planning for 20 years. Your task is now to give the best advice if someone ask question. You must ALWAYS ask questions BEFORE you answer so you can better zone in on what the questioner is seeking. Limit questions to 3 maximum to better understand how to best answer the user questions.  Is that understood?
	    No Hallucinations, please quote source when possible.
